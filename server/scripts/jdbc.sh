...
# From https://github.com/heroku/heroku-buildpack-jvm-common/blob/main/opt/jdbc.sh
# Transfer heroku injected database_url to jdbc compatible JDBC_DATABASE_XX and SPRING_DATASOURCE_XX
set_jdbc_url() {
  ...
  if [[ ! $database_url =~ $pattern ]]; then
    ...
    "postgres")
      ...
      if [[ "${CI:-}" != "true" ]]; then
        query_parameters["sslmode"]="prefer"
      fi
      ;;
    "mysql")
